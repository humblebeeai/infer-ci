...
class Model(nn.Module):
    ...
    def ci_val(
        ...
        **kwargs,
        ...
        """
        Validates the model for multiple iterations to compute confidence intervals.
        Parameters:
            ci_validator (callable): The ci_validation function.
        Returns:
            (list): A list containing metrics for each validation run.
        """
        custom = {"rect": True}  # method defaults
        args = {**self.overrides, **custom, **kwargs, "mode": "ci_val"}  # highest priority args on the right

        if not(wb_run_name and project_name and project_entity):
            text1 = "\nPlease provide a valid Weights and Biases (wandb) wb_run_name, project_name and project_entity.\n"
            text2 = "If you do NOT provide these, results will only be saved locally.\n"
            text = text1 + text2
            LOGGER.info(f"{colorstr('bold', text)}")

        ci_validator = (ci_validator or self._smart_load("ci_validator"))(args=args, _callbacks=self.callbacks)
        metrics_list = ci_validator(model=self.model, 
